...
def home():
    ...
    try:
        ...
        if Receipt.get(
            Receipt.user == current_user.email,
            Receipt.status == "Unpaid",
            Receipt.time_in <= int(datetime.timestamp(datetime.now())) + 300,
        ):
            permission = True
    except:
        permission = False
    ...
    try:
        if Receipt.select().where(
            Receipt.user == current_user.email, Receipt.status == "Unpaid"
        ):
    ...
    elif request.args.get("check_in_time") is None and c_o_c_i:
        check_in_time = c_o_c_i
    ################################################################
    if request.method == "POST":
        Feedback.insert(
            fname=request.form["name"], comment=request.form["comment"]
        ).execute()
        return render_template(
            "index.html",
            logged_in=current_user.is_authenticated,
            message="Comment has been successfully sent 👍",
            permission_checkout=permission,
            check_in_time=check_in_time,
            permission_manage_reservations=permission_manage_reservations,
        )
    return render_template(
        "index.html",
        logged_in=current_user.is_authenticated,
...
def login():
    ...
    # Set any passed messages from the routes
    if request.args.get('message'):
        ...
        if check_password_hash(user.password, form_password):
            ...
            try:
                ...
                receipt = Receipt.get(
                    ...
                    Receipt.status == "Unpaid",
                    Receipt.time_in
                    <= int(datetime.timestamp(datetime.now())) + 600,
                )
                check_in_time = receipt.time_in
                print(f"CO===Login={check_in_time}")
                global c_o_c_i
                c_o_c_i = check_in_time
                return redirect(
                    url_for("check_out", check_in_time=check_in_time)
                )
    ...
    return render_template(
        "login.html", logged_in=current_user.is_authenticated, error=error
    )
...
def register():
    ...
    return render_template(
        "sign-up.html", logged_in=current_user.is_authenticated
    )
...
def manage_reservations():
    ...
    reservations = []
    for reservation in Receipt.select().where(
        Receipt.user == current_user.email,
        Receipt.status == "Unpaid",
        Receipt.time_in >= int(datetime.timestamp(datetime.now())),
    ):
        business_id = (
            ParkingSpot.select()
        ...
        new_reservation = {
            ...
            "location_name": Location.select()
            .where(Location.id == business_id)
            .get()
            .name,
        }
...
def manage_reservations_cancel():
    ...
    try:
        # Deletes the receipt
        q = Receipt.delete().where(
            Receipt.user == current_user.email,
            Receipt.status == "Unpaid",
            Receipt.spot_id == spot_id,
        )
        q.execute()
        ...
        return redirect(
            url_for(
                "check_out",
                check_in_time=check_in_time,
                message="Receipt seems not to exist",
            )
        )
...
def check_in():
    if request.method == "POST":
        return redirect(
            url_for(
                "search_by_location", location=request.form["location"].upper()
            )
        )
    return render_template(
        "check-in.html",
...
def reports():
    locations_names = {
        location.id: location.name for location in Location.select()
    }
    spots = []
...
def calendar(location_id):
    ...
    if request.method == "POST":
        date, time = request.form.get("datetime").split(" ")[
            0
        ], twelve_to_twentyfour(
            request.form.get("datetime").split(" ")[1]
            + request.form.get("datetime").split(" ")[2]
        )
        ...
        if scheduled_date < datetime.now():
            ...
            return redirect(
                url_for(
                    ...
                    check_in_time=[year, month, day, hour, minute, second],
                )
            )
    return render_template(
        "calendar.html", logged_in=current_user.is_authenticated, error=error
    )
...
def reserve(location_id):
    error = None
    year, month, day, hour, minute, second = request.args.getlist(
        "check_in_time"
    )
    check_in_time = None

    if request.method == "GET":
        spots_in_location = ParkingSpot.select().where(
            ParkingSpot.business_id == location_id
        )

        # Stores the OPEN PARKING SPOTS IDS
        open_spots_ids = [
            spot.id for spot in spots_in_location if spot.type == "Open"
        ]
        ...
        try:
            ...
            # Checks if there is an existing receipt around the scheduled time
            try:
                spot = Receipt.get(
                    Receipt.user == current_user.email,
                    Receipt.status == "Unpaid",
                    Receipt.time_in <= check_in_time + 600,
                )
                return redirect(
                    url_for(
                        "calendar",
                        location_id=location_id,
                        error="Reservation already made around the time!",
                    )
                )
            except Receipt.DoesNotExist:
                pass
            ...
            q = ParkingSpot.update(type="Reserved").where(
                ParkingSpot.id == open_spots_ids[0]
            )
            q.execute()

            Receipt.insert(
                user=current_user.email,
                spot_id=open_spots_ids[0],
...
def check_out():
    ...
    reserved_spot_name = (
        ...
        .where(
            Location.id
            == ParkingSpot.select()
            .where(ParkingSpot.id == reserved_spot_id)
            .get()
            .business_id
        )
        .get()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            q = Receipt.update(time_out=check_out_time, status="Paid").where(
                Receipt.user == email, Receipt.time_in == check_in_time
            )
            q.execute()
            ...
            spot_id = [
                spot.spot_id
                for spot in Receipt.select().where(
                    Receipt.user == email, Receipt.time_in == check_in_time
                )
            ][0]
            q = ParkingSpot.update(type="Open").where(
            ...
            q = Receipt.update(price=price).where(
                Receipt.time_in == check_in_time, Receipt.user == email
            )
            q.execute()

            id = f"{email}{check_out_time}"
            # Updates PRODUCT NAME IN RECEIPT DB
            q = Receipt.update(product_name=id).where(
                Receipt.user == email, Receipt.time_out == check_out_time
            )
            q.execute()
            ...
            try:
                ...
                product_id = stripe.Product.search(
                    query="name:'{}'".format(id)
                )["data"][0]["id"]
            ...
            return render_template(
                ...
                spot_name=reserved_spot_name,
                check_in_time=check_in_time,
                formatted_check_in_time=datetime.fromtimestamp(check_in_time),
            )
    ...
    return render_template(
        ...
        spot_id=reserved_spot_id,
        spot_name=reserved_spot_name,
        cancel_booking_permission=cancel_booking,
        check_in_time=check_in_time,
        formatted_check_in_time=datetime.fromtimestamp(check_in_time),
    )
...
def add_location():
    if request.method == "POST":
        ...
        location_id = (
            Location.select()
            .where(Location.name == request.form["location"].upper())
            .get()
        )
...
def post_feedback():
    # Inserting data
    Feedback.insert(
        fname=current_user.fname, comment=request.args.get("comment")
    ).execute()
    return redirect(url_for("check_in"))
    ...
    if request.method == "POST":
        ...
        try:
            if User.get(User.email == request.form["email"]):
                return redirect(
                    url_for("change_password", email=request.form["email"])
                )
        except:
            error = "User does not exist"
            return redirect(url_for("recover_password", error=error))
    return render_template("recover-password.html", error=error)
...
def change_password(email):
    ...
    print(f"========{email}")
    # Sets error to argument error if it exists
    if request.args.get('error'):
        error = request.args.get('error')
    ...
    if request.method == "POST":
        if request.form["password_confirm"] == request.form["password"]:
            ...
            # Updating password
            q = User.update(password=password_hashed).where(
                User.email == email
            )
            q.execute()
            ...
            return redirect(
                url_for("change_password", email=email, error="Passwords do not match!")
            )
    return render_template(
        "change-password.html", error=error
    )
