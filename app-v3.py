...
def home():
    ...
    try:
        ...
        if Receipt.get(Receipt.user == current_user.email, Receipt.status == 'Unpaid', Receipt.time_in <= int(datetime.timestamp(datetime.now())) + 300):
            permission = True
    except:
        permission = False
    ...
    if request.method == 'POST':
        ...
        return render_template('index.html', logged_in=current_user.is_authenticated, message='Comment has been successfully sent 👍', permission_checkout=permission, check_in_time=check_in_time, permission_manage_reservations=permission_manage_reservations)
    return render_template('index.html', logged_in=current_user.is_authenticated, permission_checkout=permission, check_in_time=check_in_time, permission_manage_reservations=permission_manage_reservations, message=message)
    ...
    if request.method == 'POST':
        ...
        if check_password_hash(user.password,form_password):
            ...
            try:
                ...
                receipt = Receipt.get(Receipt.user == current_user.email, Receipt.status == 'Unpaid', Receipt.time_in <= int(datetime.timestamp(datetime.now())) + 600)
...
def reserve(location_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            try:
                spot = Receipt.get(Receipt.user == current_user.email, Receipt.status == 'Unpaid', Receipt.time_in <= check_in_time + 600)
                return redirect(url_for('calendar', location_id=location_id, error='Reservation already made around the time!'))
            except Receipt.DoesNotExist:
                pass
